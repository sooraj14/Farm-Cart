...
namespace FarmCart.Pages.Cart
{
    public class CartandOrderModel : PageModel
    {
       ...
       *//* public List<Product> prodlist { get; set; } = new List<Product>();
        ...
        public List<Products> prodds { get; set; } = new List<Products>();
        [BindProperty]
       
        [BindProperty]
        public Products productfeedback { get; set; }*//*
        public bool IsBuyNow { get; set; }
        public prodsfeed SingleProduct { get; set; }
        ...
        public void OnGet(int product_id, bool buyNow = false)
        {
            ...
            if (buyNow)
            {
                ...
                SingleProduct = new prodsfeed
                {
                    Product = _context.producttable.FirstOrDefault(p => p.product_id == product_id),
                    Prodsfeed = _context.feedbacks.Where(f => f.product_id == product_id).ToList()
                };
                IsBuyNow = true;
            }
            else
            {
                ...
                prodsfeeds = _context.producttable
                    ...
                    {
                        Product = p,
                        Prodsfeed = _context.feedbacks.Where(f => f.product_id == p.product_id).ToList()
                    })
                    .ToList();
            }
        }

        *//* prodds = (from p in _context.producttable
                      join f in _context.feedbacks on p.product_id equals f.product_id
                     where p.product_id== (int)pid

                     select new prodsfeed
                     {
